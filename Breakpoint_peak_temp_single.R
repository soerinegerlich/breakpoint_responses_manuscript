...
Peak_temp <- ggplot(df_summary_temp)+
...
Peak_temp <- ggplot(df_significant)+
  geom_point(mapping=aes(Slope1, Slopediff, group = interaction(Plot,SpeciesID), 
                         shape = Habitat, color = SpeciesID),size = 6, alpha = 0.8)+
  geom_hline(yintercept = 0)+
  geom_vline(xintercept = 0)+
  ylab("")+
  xlab("")+
  # ggtitle("Temperature as single predictor")+
  scale_shape_manual(values = c(15, 16, 17, 18)) +
  scale_color_manual(values = c("darkseagreen3", "darkseagreen4", "chartreuse", 
