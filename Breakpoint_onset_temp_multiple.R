...
colnames(df_phen_event_50)[5] <- "phenology"
colnames(df_phen_event_50)[8] <- "x"
colnames(df_phen_event_50)[11] <- "z"
...
#require(writexl)
#write_xlsx(df_summary_all, "Data/Summary_tables\\df_summary_temp_snow_50_peak.xlsx", col_names = TRUE)

######## Read summary excel file to do a bit of data clean up #########

df_summary_temp_onset <-
  read_xlsx("Data/Summary_tables/df_summary_onset_temp_snow.xlsx")

length(which(df_summary_temp_onset$Pvalue < 0.06)) / length(df_summary_temp_onset$Pvalue)

df_summary_temp_onset <-
  subset(
    df_summary_temp_onset,
    df_summary_temp_onset$Plot != "Art1" |
      df_summary_temp_onset$SpeciesID != "Collembola"
  )
df_summary_temp_onset <-
  subset(
    df_summary_temp_onset,
    df_summary_temp_onset$Plot != "Art1" |
      df_summary_temp_onset$SpeciesID != "Acari"
  )

#df_summary_temp$Slope2 <- df_summary_temp$Slope1 + df_summary_temp$Slopediff
#df_summary_temp$SEslope2 <- df_summary_temp$SEslopediff - df_summary_temp$SEslope
...
df_summary_temp_onset$Plot[df_summary_temp_onset$Plot == "Art1"] <-
  "Plot 1"
df_summary_temp_onset$Plot[df_summary_temp_onset$Plot == "Art2"] <-
  "Plot 2"
df_summary_temp_onset$Plot[df_summary_temp_onset$Plot == "Art3"] <-
  "Plot 3"
df_summary_temp_onset$Plot[df_summary_temp_onset$Plot == "Art4"] <-
  "Plot 4"
df_summary_temp_onset$Plot[df_summary_temp_onset$Plot == "Art5"] <-
  "Plot 5"
df_summary_temp_onset$Plot[df_summary_temp_onset$Plot == "Art6"] <-
  "Plot 6"
df_summary_temp_onset$Plot[df_summary_temp_onset$Plot == "Art7"] <-
  "Plot 7"


df_summary_temp_onset$SpeciesID[df_summary_temp_onset$SpeciesID == "CHCE"] <-
  "Chironomidae"
df_summary_temp_onset$SpeciesID[df_summary_temp_onset$SpeciesID == "ANMU"] <-
  "Muscidae"
df_summary_temp_onset$SpeciesID[df_summary_temp_onset$SpeciesID == "MYSC"] <-
  "Sciaridae"

write_xlsx(df_summary_temp_onset, "Data/Summary_tables\\df_summary_onset_temp_snow_final.xlsx", col_names = TRUE)
