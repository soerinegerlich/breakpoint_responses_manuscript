...
#require(writexl)
#write_xlsx(df_summary_all, "Data/Summary_tables\\df_summary_snow_peak.xlsx", col_names = TRUE)
...
par(mfrow=c(1,2))
hist(slope1,xlab="slope 1",breaks=20,main="")
abline(v= mean(df_summary_snow$Slope1),col=1,lty=2)

hist(df_summary_snow$Slopediff,xlab="slope difference",breaks=20,main="")
abline(v= mean(df_summary_snow$Slopediff),col=1,lty=2)
...
Peak_snow <- ggplot(df_summary_snow)+
...
Peak_snow <- ggplot(df_significant)+
  geom_point(mapping = aes(Slope1, Slopediff, group = interaction(Plot,SpeciesID), shape = Habitat, color = SpeciesID), size = 6, alpha = 0.8)+
  geom_hline(yintercept = 0)+
  geom_vline(xintercept = 0)+
  ylab("")+
  ...
  scale_color_manual(values = c("darkseagreen3", "darkseagreen4", "brown4", "darkorange",
                                "darkgoldenrod1", "darkorange3", "yellow", "gold", "blue"))+
