#Nonlinearity in end of activity to snowmelt
...
df_summary_all<-data.frame(SpeciesID=character(),Plot=character(),Intercept=numeric(),Slope1=numeric(),Slopediff=numeric(),
                           ...
                           CI_lwr=numeric(), CI_upr=numeric(), n=numeric(), AIC_0=numeric(), AIC_1=numeric(),n=numeric(),OriginalSlope=numeric(),
                           OriginalSE = numeric())
...
#require(writexl)
#write_xlsx(df_summary_all, "Data/Summary_tables\\df_summary_end_snow_new.xlsx", col_names = TRUE)


df_summary_snow <- read_xlsx("Data/Summary_tables/df_summary_end_snow_new.xlsx")

#df_summary_snow$SEslope2 <- df_summary_snow$SEslopediff - df_summary_snow$SEslope
...
df_summary_snow$SpeciesID[df_summary_snow$SpeciesID == "CHCE"] <- "Chironomidae"
df_summary_snow$SpeciesID[df_summary_snow$SpeciesID == "ANMU"] <- "Muscidae"
df_summary_snow$SpeciesID[df_summary_snow$SpeciesID == "MYSC"] <- "Sciaridae"


#write_xlsx(df_summary_snow, "Data/Summary_tables\\df_summary_end_snow_final_new.xlsx", col_names = TRUE)
...
df_summary_snow$SpeciesID <- factor(df_summary_snow$SpeciesID,                 # Relevel group factor
...
End_snow <- ggplot(df_summary_snow)+
...
df_summary_snow$Significance_level <- ifelse(df_summary_snow$Pvalue<0.06, "True", "False")

df_significant <- subset(df_summary_snow, Significance_level == "True")

End_snow <- ggplot(df_significant)+
  geom_point(mapping = aes(Slope1, Slopediff, group = interaction(Plot,SpeciesID), shape = Habitat, color = SpeciesID), size = 6, alpha = 0.8)+
  geom_hline(yintercept = 0)+
  geom_vline(xintercept = 0)+
  ylab("")+
  xlab("Slope of first linear segment")+
  #ggtitle("Temperature as single predictor - Peak")+
  scale_shape_manual(values = c(15, 16, 17, 18)) +
  scale_color_manual(values = c("darkseagreen3", "darkseagreen4", "brown4", "darkorange",
                                "gold", "blue"))+
