...
bkpr<-function(formula, bpv="x", data, nsim=1000,minlength){
  ...
  return(list(model=m1,meanslope1=mean(slope1),se.slope1=sd(slope1),meanslopediff=mean(slopediff),se.slopediff=sd(slopediff), breakpoint=MLBP, pval=1-sum((logLik(m1)-logLik(mnull))>LLdiff)/nsim,model_se=summary(m1)$coefficients[,"Std. Error"],
              ci.slope1=confint(m1, level = 0.95), AIC_0=AIC(m0), AIC_1=AIC(m1), OriginalSlope=summary(m0)$coefficients[2,"Estimate"], OriginalSE=summary(m0)$coefficients[2,"Std. Error"]))
}
...
for(i in unique(df_phen_event$SpeciesID)){
  ...
  for (j in unique(df1$Plot)) {
    ...
    if (length(df2$Year) < 7) {
      ...
      store.output<-bkpr(phenology~x+z, bpv="x", data=df2, nsim=100,minlength=2)
...
write_xlsx(df_summary_all, "Data/Summary_tables\\df_summary_peak_temp_snow_new.xlsx", col_names = TRUE)

######## Read summary excel file to do a bit of data clean up #########

df_summary_temp_peak <- read_xlsx("Data/Summary_tables/df_summary_peak_temp_snow_new.xlsx")
...
df_summary_temp_peak <-
  subset(
    df_summary_temp_peak,
    df_summary_temp_peak$Plot != "Art1" |
      df_summary_temp_peak$SpeciesID != "Collembola"
  )
...
df_summary_temp_peak <-
  subset(
    df_summary_temp_peak,
    df_summary_temp_peak$Plot != "Art1" |
      df_summary_temp_peak$SpeciesID != "Acari"
  )
...
df_summary_temp_peak %>%
  mutate(Order = case_when(
    ...
    SpeciesID == "Thomisidae" ~ "Predator")) -> df_summary_temp_peak
...
write_xlsx(df_summary_temp_peak, "Data/Summary_tables\\df_summary_peak_temp_snow_final_new.xlsx", col_names = TRUE)
